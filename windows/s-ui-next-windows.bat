...
REM S-UI Next Windows Control Script
REM This script provides a menu-driven interface for managing S-UI Next on Windows

cd /d "%~dp0"
set "SERVICE_NAME=s-ui-next"
set "INSTALL_DIR=%SUI_HOME%"
if "%INSTALL_DIR%"=="" set "INSTALL_DIR=C:\Program Files\s-ui-next"

:menu
cls
echo ========================================
echo S-UI Next Windows Control Panel
echo ========================================
echo.
echo Current directory: %INSTALL_DIR%
echo.
echo 1. Start S-UI Next Service
echo 2. Stop S-UI Next Service
echo 3. Restart S-UI Next Service
echo 4. Check Service Status
echo 5. View Service Logs
echo 6. Open Panel in Browser
echo 7. Run S-UI Next Manually
...
:stop_service
echo Stopping S-UI Next service...
...
:restart_service
echo Restarting S-UI Next service...
...
:check_status
echo Checking S-UI Next service status...
...
:open_panel
echo Opening S-UI Next panel in browser...
...
:run_manual
echo Running S-UI Next manually...
if exist "%INSTALL_DIR%\sui.exe" (
    cd /d "%INSTALL_DIR%"
    echo Starting S-UI Next in current window...
    echo Press Ctrl+C to stop
    echo.
    sui.exe
) else (
    echo S-UI Next executable not found: %INSTALL_DIR%\sui.exe
...
if exist "%INSTALL_DIR%\s-ui-next-service.exe" (
    cd /d "%INSTALL_DIR%"
...
if exist "%INSTALL_DIR%\s-ui-next-service.exe" (
    cd /d "%INSTALL_DIR%"
    net stop %SERVICE_NAME% >nul 2>&1
    s-ui-next-service.exe uninstall
    if %errorLevel% equ 0 (
...
if exist "%INSTALL_DIR%\sui.exe" (
    ...
    echo S-UI Next executable not found. Please run the installer first.
)
pause
goto menu
...
:exit
echo Thank you for using S-UI Next Windows Control Panel!
exit /b 0
