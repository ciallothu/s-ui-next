...
S-UI Next 是基于原 sing-box 面板继续开发的下游项目，保留原有管理模型，同时加入版本化 API、移动端 App、更完整的身份认证、更细的统计日志以及更安全的 WireGuard 管理。以下内容由本项目持续维护：
...
- 全新的 WireGuard Endpoint 管理：服务端地址归属与客户端路由彻底分离、安全 PSK 生成与脱敏、安全的分流默认值、IPv4/IPv6 校验、独立的客户端连接地址、受控配置/二维码导出、经 S-UI Next 中转的设备互访、站点网关路由以及失败回滚。

移动端源码位于 [`mobile/`](mobile/README.md)。Android arm64 和未签名 iPhone arm64 产物全部通过 GitHub Actions 构建。

## WireGuard 配置要点

- **服务端 Endpoint 地址**代表 S-UI Next 自身，通常使用 IPv4 `/32` 和 IPv6 `/128`，例如 `10.66.66.1/32`、`fd66:66:66::1/128`。
...
- **站点网关**代表一个远端网关及其后方的一个或多个 LAN 网段。服务端运行时 Peer 的 AllowedIPs 会包含网关隧道地址和远端站点网段；导出给远端网关的客户端配置会包含 S-UI Next WireGuard 虚拟网段和配置的本地站点网段，不会把远端站点自己的 LAN 错误导回隧道。
- **PSK 与私钥**由后端安全随机源生成，普通 API 资源查询只返回脱敏占位符；保存脱敏表单不会丢失旧密钥，只有主动生成、复制或导出时才显示真实值。
- **同一 WireGuard 网络内设备互访**通过受管路由表注入规则，含义是经 S-UI Next 服务器中转到同一 Endpoint 下的其他设备，并不是设备直连、P2P 或 NAT 穿透。不会重复用户已有的等价规则，关闭时也不会删除用户规则。
...
在“设置 → 登录与身份认证”中启用，然后从“管理员 → 登录安全”添加。通常可以留空 RP ID 和 Origins，S-UI Next 会根据浏览器 Origin 以及 `Forwarded`、`X-Forwarded-Host`、`X-Forwarded-Proto` 自动识别反代后的管理域名。特殊代理结构可手动指定完整 HTTPS Origin 和仅包含域名的 RP ID。
