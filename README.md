...
- 分开管理本机 Endpoint 地址与监听、对端连接方式、运行时 AllowedIPs、虚拟分配网段，以及客户端配置导出。
- 私钥和 PSK 使用安全随机源生成。普通资源接口只返回脱敏值，保存脱敏表单时会保留原有密钥。
- 客户端配置和二维码必须通过明确的导出操作生成。
- 可以生成由面板管理的客户端，也可以只填写另一台 VPS 或已有设备的公钥，不要求由面板持有对方私钥。
- 支持监听并动态识别对端、主动连接固定对端、双 VPS 中转、全局出口，以及带本地和远端网段的站点互联。
- 运行时路由与导出给客户端的路由各自独立；全局路由必须显式选择，避免无意接管流量。
...
Cloudflare WARP 可以直接创建为 S-UI Next 出口，无需在服务器另行安装 WARP 客户端或转接 SOCKS。创建前需要确认 Cloudflare 条款；设备令牌、许可证和隧道私钥保存后不会在资源接口中明文回显。
